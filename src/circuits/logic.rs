...
impl<const WORD_BITS: u32> LogicConfig<WORD_BITS> {
    ...
    pub fn configure<F: FieldExt>(
        ...
        let conf @ Self {
            ...
            odd_sum,
            res,
        } = Self::new(meta, even_bits, s_table, s_and, s_xor, a, b, res);
    ...
    for LogicCircuit<Fp, WORD_BITS, OP>
{
    ...
    fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
        ...
        let a =
            EvenBitsConfig::configure(meta, a, &[s_and, s_xor], s_table, even_bits);

        let b = meta.advice_column();

        (
            LogicConfig::<WORD_BITS>::configure(
                meta, even_bits, s_table, s_and, s_xor, a, b, res,
            ),
            instance,
        )
    }
